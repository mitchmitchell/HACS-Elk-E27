...
def test_attribs_request_queue_respects_limit() -> None:
    client = Elke27Client()
    kernel = client._kernel
    sent: list[tuple[tuple[str, str], dict]] = []

    def _request(route, **kwargs):
        sent.append((route, dict(kwargs)))
        return 1

    kernel.request = _request  # type: ignore[assignment]
...
def test_client_queues_output_attribs_on_inventory_ready() -> None:
    client = Elke27Client()
    inv = client._kernel.state.inventory
    inv.configured_outputs = {1, 2}
    ...
    client._kernel.request = _request  # type: ignore[assignment]
...
def test_zone_configured_ignores_bitmask_when_paged() -> None:
    ...
    warnings: list[str] = []
    ids = zone_handler._extract_configured_zone_ids(payload, warnings)
    assert ids == [1, 2]
...
def test_area_configured_ignores_bitmask_when_paged() -> None:
    ...
    warnings: list[str] = []
    ids = area_handler._extract_configured_area_ids(payload, warnings)
    assert ids == [1, 2]
    assert any("bitmask ignored" in w for w in warnings)
