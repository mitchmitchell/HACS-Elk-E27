...
async def test_live_area_chime_state_roundtrip(live_e27_client: Elke27Client) -> None:
    ...
    snapshot_available = area is not None and area.chime is not None
    original = bool(area.chime) if snapshot_available else None
    ...
    try:
        result = await live_e27_client.async_execute("area_set_status", area_id=area_id, chime=target)
        if result.ok:
            assert_payload_shape("area_set_status", result.data)
        ...
        result = await live_e27_client.async_execute("area_set_status", area_id=area_id, chime=original)
        if result.ok:
            assert_payload_shape("area_set_status", result.data)
