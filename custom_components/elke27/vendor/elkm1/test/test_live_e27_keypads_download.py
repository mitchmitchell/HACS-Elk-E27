...
async def test_live_keypads_download(live_e27_client: Elke27Client) -> None:
    table_info = await live_e27_client.async_execute("keypad_get_table_info", timeout_s=_LIVE_TIMEOUT_S)
    ...
    if not configured.ok:
        ...
        if error_code == E27ErrorCode.ELKERR_INVALID_TABLE:
            pytest.fail("keypad_get_configured failed with invalid_table despite allocated keypad table.")
    ...
    keypad_ids = [keypad_id for keypad_id in keypad_ids if isinstance(keypad_id, int) and keypad_id >= 1]
    if not keypad_ids:
        pytest.skip("No configured keypads reported.")
    ...
    for keypad_id in keypad_ids:
        ...
        if result.ok:
            ...
            keypad = live_e27_client._kernel.state.keypads.get(keypad_id)
