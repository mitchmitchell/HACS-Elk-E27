...
def test_domain_empty_dict_routes_to_empty_and_emits_error():
    ...
    err_msg = e.calls[0][0]
    payload = err_msg["__error__"][ERR_DOMAIN_EMPTY].get("payload")
...
def test_root_empty():
    ...
    assert len(e.calls) == 1
    err_msg = e.calls[0][0]
    payload = err_msg["__error__"][ERR_ROOT_EMPTY].get("payload")
    assert isinstance(payload, str)
...
def test_paged_reassembly_out_of_order() -> None:
    ...
    def _merge(blocks, block_count):
        merged = []
        for block in blocks:
            merged.extend(block.payload.get("zones", []))
    ...
    msg2 = {"seq": 11, "session_id": 1, "zone": {"get_configured": {"block_id": 2, "block_count": 2, "zones": [3]}}}
    d.dispatch(msg2)
    assert len(r.calls) == 0

    msg1 = {"seq": 10, "session_id": 1, "zone": {"get_configured": {"block_id": 1, "block_count": 2, "zones": [1, 2]}}}
    d.dispatch(msg1)
    assert len(r.calls) == 1
    assembled = r.calls[0][0]
    assert assembled["zone"]["get_configured"]["zones"] == [1, 2, 3]
