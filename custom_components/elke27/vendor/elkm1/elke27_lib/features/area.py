...
def build_area_get_attribs_payload(*, area_id: int, **kwargs: Any) -> Mapping[str, Any]:
    if not isinstance(area_id, int) or area_id < 1:
        raise ValueError(f"build_area_get_attribs_payload: area_id must be int >= 1 (got {area_id!r})")
    return {"area_id": area_id}


def build_area_get_configured_payload(*, block_id: int = 1, **kwargs: Any) -> Mapping[str, Any]:
    if not isinstance(block_id, int) or block_id < 1:
        raise ValueError(f"build_area_get_configured_payload: block_id must be int >= 1 (got {block_id!r})")
