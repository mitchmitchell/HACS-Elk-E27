...
def _panel_host_port(panel: discovery.E27System | Mapping[str, Any]) -> tuple[str, int]:
    ...
    if not isinstance(port, int) or port <= 0 or port > 65535:
        raise KernelInvalidPanelError(f"Discovered panel entry has invalid port={port!r}: {panel!r}")

    return host, port


def _client_identity_string(client_identity: linking.E27Identity) -> str:
    """
    Session HELLO client_identity string.
    ...
    return f"{client_identity.mn}:{client_identity.sn}"
...
class E27Kernel:
    ...
    async def link(
        ...
        *,
        ...
        if client_identity is None:
            raise KernelError("link(): client_identity is required.")
    ...
    async def connect(
        ...
        *,
        ...
        if not isinstance(link_keys, linking.E27LinkKeys):
            raise KernelError("connect(): link_keys must be an instance of linking.E27LinkKeys.")
        missing_fields: list[str] = []
        for field in ("linkkey_hex", "linkhmac_hex"):
            if not getattr(link_keys, field, None):
                missing_fields.append(field)
        if missing_fields:
            raise KernelError(f"connect(): link_keys missing required fields: {', '.join(missing_fields)}")

        link_key_hex = link_keys.linkkey_hex

        if not isinstance(client_identity, linking.E27Identity):
            raise KernelMissingContextError(
                "connect() requires client_identity (Elke27Identity with mn/sn) for HELLO."
            )
        if not client_identity.mn or not client_identity.sn:
            raise KernelMissingContextError(
                "connect() requires client_identity with mn and sn."
            )
        ...
        if s.state != session_mod.SessionState.ACTIVE:
            raise KernelError(f"Session connect completed but session.state is {s.state!r}, not ACTIVE.")
    ...
    async def reconnect(self) -> session_mod.SessionState:
        ...
        """
        if self._last_link_keys is None or self._last_client_identity is None or self._last_session_config is None:
        ...
        while True:
            ...
            if self._keepalive_missed >= self._keepalive_max_missed:
                if self._session is not None:
                    self._session._handle_disconnect(  # type: ignore[attr-defined]
                        session_mod.SessionProtocolError("Keepalive timed out")
                    )
                return
    ...
    def _is_valid_attrib_id(self, domain: str, entity_id: int) -> bool:
        if not isinstance(entity_id, int) or entity_id < 1:
            return False
        inv = self.state.inventory
        if domain == "area" and inv.area_discovery_max_id is not None and entity_id > inv.area_discovery_max_id:
            return False
        if domain == "zone" and inv.zone_discovery_max_id is not None and entity_id > inv.zone_discovery_max_id:
            return False
        ...
        if domain == "zone" and inv.configured_zones and entity_id not in inv.configured_zones:
            return False
        if domain == "output" and inv.configured_outputs and entity_id not in inv.configured_outputs:
            return False
        if domain == "user" and inv.configured_users and entity_id not in inv.configured_users:
            return False
        if domain == "keypad" and inv.configured_keypads and entity_id not in inv.configured_keypads:
            return False
    ...
    def _try_send_next(self) -> None:
        ...
        if item.pending:
            opaque = item.opaque
            if opaque is None and item.expected_route is not None and self.dispatcher.is_paged(item.expected_route):
                block_id = item.payload.get("block_id") if isinstance(item.payload, Mapping) else None
    ...
    def _abort_requests(self, exc: BaseException) -> None:
        ...
        if self._request_state is _RequestState.IN_FLIGHT and active_seq is not None and not self._active_released:
        ...
        *,
        ...
        if not expects_reply:
            ...
            try:
                ...
                self._mark_send_failed(seq, exc)
                raise KernelError(f"Failed to send request {domain}.{name} seq={seq}: {exc}") from exc
            return seq

        timeout_value = float(timeout_s) if timeout_s is not None else float(self._request_timeout_s)
    ...
    def _build_request_message(self, seq: int, domain: str, name: str, payload: Any) -> dict[str, Any]:
        ...
        if name == "__root__":
            msg[domain] = dict(payload) if isinstance(payload, Mapping) else payload
    ...
    def _handle_dispatch_error_envelope(self, msg: Mapping[str, Any], ctx: DispatchContext) -> bool:
        ...
        key_list: list[str] = list(keys) if isinstance(keys, list) else []
