...
def _collect_generators(module) -> dict[str, GeneratorFn]:
...
def _stub_generator(key: str) -> GeneratorFn:
    def _stub(*args: Any, **kwargs: Any) -> tuple[dict, tuple[str, str]]:
...
__all__ = ["GENERATORS", *[f"generator_{key}" for key in sorted(ALL_PERMISSION_KEYS)]]
